...
async def api_create_achievement(body: AchievementCreate, current_user: CurrentUser):
    """Create an achievement for a student (teachers+)."""
    result = create_achievement(body.model_dump(), telegram_id=current_user.telegram_id)
    if not result:
        raise HTTPException(
